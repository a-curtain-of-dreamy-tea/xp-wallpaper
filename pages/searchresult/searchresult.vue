		...
		<view class="scroll">
			<scroll-view class="list-scroll" scroll-y="true">
				<view class="listcard-image">
						<view class="listcard-image-item" v-for="(item,index) in query_result" :key="index" @click="gotodetail(item.id)">
							 <image :src="item.image" mode="aspectFill"></image>
						</view>
				</view>
				<view v-if="seen">暂无搜索结果</view>
			</scroll-view>
		</view>
...
<script>
	import { get_search_hot_tags,get_query } from '../../common/api'
	...
	export default {
		...
		onLoad(options) {
			...
			this.get_query_api(options.keyword)
		},
		methods: {
			doSearch(e){
				console.log(e,444);
			...
			get_query_api(keyword){
				this.seen = false
				get_query(keyword).then((res)=>{
					this.query_result = res
					if(res.length < 1){
						this.seen = true
					}
				})
